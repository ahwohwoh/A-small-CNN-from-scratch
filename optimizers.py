...
class SGD(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        #############################################################
            self.moments[k] = self.momentum * prev_moments[k] + (1-self.momentum) * xs_grads[k]
...
class Adam(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        #############################################################
            ...
            m = self.moments[k]/(1-self.beta_1**t)
            v = self.accumulators[k]/(1-self.beta_2**t)
            new_xs[k] = xs[k] - self.lr * m/((v)**0.5 + self.epsilon)
        t+=1
        return new_xs
